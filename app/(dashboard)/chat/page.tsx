...
const DEMO_MESSAGES: Msg[] = [
  {
    ...
    avatar: '/me/me1.png',
    ciphertext:
      'TE6iTYGZ7yEwY8CvYH0QkYrYRhv7TPvC/1bE+Ab3A2e1A4Ph0nZGqySimTqekwN2yAh9Q/4lA4hJumsSeGtEGAC70x7NZG+qbLnL5na37kK0rz0X9zMKPAFDeeWLwsvL9QFy9KQbhnDjruYdz7bk4+grHZybEDn9Q0LwHo=',
    discussionCount: 156
  },
  {
    id: 'm2',
    ...
    avatar: '/me/me2.png',
    ciphertext:
      'QPPmdeWEyBfjISoo3WQV8ax0GMjPVlEULfuc7sKqWDg0DyS2E0G9XWX47JRUy3YkYx+HN2vYR60UmpOUrvikEcH1tg1MSHiedc+WkGW9U3Amr5x5uBIJRQKTiVVT3JGakcPMUyHj+1sKZAVR2yWlMBDpTYDWnc5pE=',
    discussionCount: 156
  },
  {
    id: 'm3',
    ...
    avatar: '/me/me1.png',
    ciphertext:
      'VY8CvYH0QkYrYRhv7TPvC/1bE+Ab3A2e1A4Ph0nZGqySimTqeTE6iTYGZ7yEkN2yAh9Q/4lA4hJumsSeGtEGAzMKPAFDeeWLwsvL9QFy9KQbhnDjruYdz7bk4+grHZy=',
    discussionCount: 48
  },
  {
    id: 'm4',
    ...
    avatar: '/me/me2.png',
    ciphertext:
      'b0DyS2E0G9XWX47JRUy3YkYx+HN2vYR60UmpOUrvikEcH1tg1MSHiedc+WkGW9U3Amr5x5uBIJRQKTiVVT3JGakcPMUyHj+1sKZAQPPmdeWEyBfjISoo3WQV8ax0GMjPVlEULfuc7sKqW=',
    discussionCount: 102
  },
  {
    id: 'm5',
    ...
    avatar: '/me/me1.png',
    ciphertext:
      'QbhnDjruYdz7bk4+grHZybEDn9Q0LwHoTE6iTYGZ7yEwY8CvYH0QkYrYRhv7TPvC/1bE+Ab3A2e1A4Ph0nZGqySimTqe=',
    discussionCount: 11
  },
  {
    id: 'm6',
...
export default function ChatDetailPage() {
  ...
  const [showDecrypt, setShowDecrypt] = useState<{
    open: boolean;
    msgId?: string;
  }>({ open: false });
  ...
  const handleDoDecrypt = (
    ...
    setMsgs((prev) =>
      prev.map((m) =>
        m.id === msgId ? { ...m, plaintext: demoPlaintextFor(msgId) } : m
      )
    );
    onClose();
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2">
        <button className="px-3 py-1 text-[12px] rounded-full bg-[#F5F6F7] border border-gray-200">
          BNB Chain
        </button>
        <appkit-button />
      ...
      <div className="flex items-center justify-between px-3 py-3 border-b">
        ...
        <div className="flex-1 text-center text-base font-medium truncate px-2">
          {chatTitle}
        </div>
        ...
        <div className="h-full overflow-y-auto overscroll-contain">
          <div className="px-3 pb-28 space-y-3">
            {msgs.map((m) => (
              <div
                key={m.id}
                className={`flex ${m.fromMe ? 'justify-end' : 'justify-start'} items-end gap-2`}
              >
                {!m.fromMe && (
                ...
                <div
                  className={`max-w-[78%] rounded-[10px] p-3 relative ${m.fromMe ? 'bg-[#ffffff]' : 'bg-[#95ec69]'}`}
                >
                  <div className="text-[12px] break-all leading-5 text-[#444]">
                    {m.ciphertext}
                  </div>
                  {m.plaintext ? (
                    <div className="mt-2 text-[13px] text-[#222] bg-white rounded border p-2">
                      {m.plaintext}
                    </div>
                  ) : (
                    <div className="flex items-center gap-3 mt-2">
                      <button
                        className="text-[12px] px-1 py-[2px] rounded bg-[#fef1ef] text-[#606266] flex items-center"
                        onClick={() => onClickDecrypt(m.id)}
                      >
                        <Image
                      ...
                      <button
                        className="text-[12px] px-1 py-[2px] rounded bg-[#e8f7ed] text-[#606266] flex items-center"
                        onClick={() => onClickDecrypt(m.id)}
                      >
                        <Image
                          src="/chats/news.png"
      ...
      {showDecrypt.open && showDecrypt.msgId && (
        <DecryptDialog
          msgId={showDecrypt.msgId}
          onClose={() => setShowDecrypt({ open: false })}
          onConfirm={handleDoDecrypt}
        />
      )}
    </div>
  );
  ...
  return (
    <div className="fixed left-0 right-0 bottom-[80px] z-40 border-t bg-white px-2 py-2">
      <div className="mx-auto max-w-[720px] flex items-center gap-2">
        <button
          className={`p-2 rounded ${voiceMode ? 'bg-gray-100' : ''}`}
        ...
        {!voiceMode ? (
          <input
            ...
            placeholder="今天赶紧会议很顺利"
            className="flex-1 text-sm bg-gray-100 rounded-full px-3 py-2 outline-none"
          />
        ) : (
          <div className="flex-1 text-sm bg-gray-100 rounded-full px-3 py-2 text-center text-gray-600">
            按住说话
          </div>
        )}
        ...
        <button
          ...
          onClick={() =>
            alert(
              '打开“更多 +”面板（相册/拍摄/语音通话/AI/红包/转账/发货/投票）占位'
            )
          }
        >
      ...
      <div className="w-full bg-white rounded-t-xl p-4 space-y-3">
        ...
        <div className="flex items-center justify-between border rounded px-2 py-2">
          <div className="text-xs text-gray-700 truncate">
            {currentKey
              ? `${currentKey.name} · ${currentKey.publicKey.slice(0, 20)}...`
              : '暂无可用私钥'}
          </div>
          <button
        ...
        <div className="flex gap-2">
          <button className="flex-1 py-2 border rounded" onClick={onClose}>
            取消
          </button>
          <button
            className="flex-1 py-2 bg-black text-white rounded"
            onClick={() =>
              onConfirm(msgId, selectedKeyId, pass, setError, onClose)
            }
          >
            解密
          </button>
          ...
          <div className="w-full bg-white rounded-t-xl p-4">
            ...
            <div className="max-h-64 overflow-y-auto mt-3 space-y-2">
              {keys.map((k) => (
                <div
                  key={k.id}
                  className={`border rounded p-3 text-xs flex items-center justify-between ${k.id === selectedKeyId ? 'border-purple-500' : ''}`}
                >
                  <div className="flex-1">
                    <div className="font-medium">编号{k.id}</div>
                    <div className="text-gray-500 truncate">
                      {k.publicKey.slice(0, 40)}...
                    </div>
                  </div>
